...
public class XkApsItemController {
    ...
    */
    ...
    public APIResponse<XkApsItemDto> remove(@PathVariable String id) {
    ...
    */
    ...
    public APIResponse<XkApsItemDto> update(@PathVariable("id") String id,@RequestBody XkApsItemDto formData) throws Exception {
        if (null != formData) {
            return new APIResponse<XkApsItemDto>(xkApsItemService.save(formData));
        }
        return new APIResponse<XkApsItemDto>(-1, "数据传输为空");
